...
<img src="IMG/添加菜单.png" alt="新增菜单" style="zoom:50%;" />
...
默认情况下注册成功的账号自动绑定角色编号为`4`。

可通过以下配置来调整注册默认角色以及允许注册的角色范围：

```yml
tworice:
	login:
		defaultRoleId: 4 # 注册成功默认绑定的角色
		allowRole: 4, 19 # 允许注册的角色
```

在前端提交注册请求时，可提交注册的角色ID`roleId`字段，但该字段必须是允许注册的角色。
